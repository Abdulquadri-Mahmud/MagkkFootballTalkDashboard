...
export default function Sign_up() {
    ...
    const handleSubmit = async (e) => {
        ...
        try{
            ...
            if (username.current.value === '' || username.current.value === null) {
                dispatch(adminSignupFailure('Username is Required!'))
                return;
            }
            if (email.current.value === '' || email.current.value === null) {
                dispatch(adminSignupFailure('Email is Required!'));
                return;
            }
            if (password.current.value === '' || password.current.value === null) {
                dispatch(adminSignupFailure('Password is Required!'));
                return;
            }

            const signup_api = 'https://fake-api-one-rust.vercel.app/api/admin/auth/admin-signup';
  ...
  return (
    <div className='md:py-10 py-12 h-[100vh] flex justify-center items-center bg-blue-900'>
        <div className="relative bg-white sm:w-[350px] w-[97%] mx-auto p-3 rounded-md shadow-lg">
            ...
            <form onSubmit={handleSubmit} className='mt-5'>
                <div className="shadow-md rounded-md relative">
                    ...
                    <input onChange={handlChange} ref={username} id='username' type="text" placeholder='Username' className='px-2 py-3 w-full rounded-md pl-8 text-sm font-medium border outline-none bord'/>
                </div>
                <div className="shadow-md rounded-md relative mt-6">
                    <MdOutlineMarkEmailUnread className='absolute left-2 top-4'/>
                    <input onChange={handlChange} ref={email} id='email' type="text" placeholder='Example@gmail.com' className='px-2 py-3 w-full rounded-md pl-8 text-sm font-medium border outline-none bord'/>
                </div>
                <div className="mt-6 shadow-md rounded-md relative">
                    <TbPasswordUser className='absolute left-2 top-4'/>
                    <input onChange={handlChange} ref={password} id='password' type="password" placeholder='Password goes here' className='px-2 py-3 w-full rounded-md pl-8 text-sm font-medium border outline-none bord'/>
